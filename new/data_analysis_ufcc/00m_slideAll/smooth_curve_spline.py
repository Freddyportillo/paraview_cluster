...
vel_mag=np.array([59.88,79.89,90.92,96.12,97.14])
...
MagSpline = make_interp_spline(slide,vel_mag)
MagmaxSpline = make_interp_spline(slide,vel_mag+mag_range)
MagminSpline = make_interp_spline(slide,vel_mag-mag_range)
...
plt.plot(X_, Mag, linestyle='--', linewidth=0.8, color='black', label=r'$\overline{u}$')
plt.fill_between(X_,Magmin,Magmax,color='blue',alpha=0.1)

# plt.plot(slide,u,'o',color='black')
# plt.plot(slide,v,'*',color='black')
# plt.plot(slide,w,'+',color='black')
plt.plot(slide,vel_mag,'o',color='black')
...
path = '/media/alejandro/Seagate Expansion Drive/Backup_Freddy/UFCC_project/FTP_final/data_analysis/00m_allslides/fluid'
output = path+'/velmag_mag_00m_Allslides.png'
plt.savefig(output)